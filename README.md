...
```
This produces an artifact mapping to `build/libs/AegisGuard-1.0.6.jar`. Drop it directly into your `plugins` directory. Note that AegisGuard gracefully identifies the presence of `ProtocolLib` and `Floodgate` at runtime, hooking into them automatically if they are populated.
